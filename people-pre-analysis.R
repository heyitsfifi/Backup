...
repeat{
  ...
  if(option==0) {
    ...
    peoplePerRepo = dbGetQuery(gnome, "select distinct people_merged.id as 'people_id', scmlog.repository_id
                                        ...
                                        ")
    pRepo <- merge(people,peoplePerRepo,by.x='people',by.y='people_id')
    commits = dbGetQuery(gnome, "select distinct people_merged.id as 'people_id', commits_activities.commit_id
                                from people_merged
                                left outer join identity_merging on people_merged.id = identity_merging.merged_id
                                left outer join people on identity_merging.merged_id = people.id
                                left outer join scmlog on people.id = scmlog.committer_id
                                left outer join commits_activities on scmlog.author_id = commits_activities.commit_id
                                group by people_merged.id,commits_activities.commit_id;")
    write.csv(file="commits.csv", x=commits, na="0")
